            ...
            <div class="flex">
                <header class="mx-auto flex flex-row">
                    ...
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight px-6 py-5">
                        {{ $title ?? ucwords(str_replace('.', ' - ', str_replace('-', ' ', str_replace('.show', '', Route::currentRouteName())))) }}
                    </h2>
                </header>
            </div>
                        ...
                        <x-slot name="trigger">
                            @auth
                                @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                                    <button class="mr-4 flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition duration-150 ease-in-out">
                                        <img class="h-8 w-8 rounded-full object-cover" src="{{ Auth::user()->profile_photo_url }}" alt="{{ Auth::user()->name }}" />
                                    </button>
                                @else
                                    <span class="mr-4 inline-flex rounded-md">
